...
iset = "C:/Users/astra/Documents/2021/1-Projects/Programming/Coffee/2019-09-29 coffee"
...
class ImageFrame(wx.Frame):
    ...
    def ReloadTargets(self):
        self.targets = []
        for box in dbh.execute('select x1, y1, x2, y2 from targets where file=? order by x1, y1',
    ...
    def ZoomModeWrite(self):
        if self.box:
            self.targets.append(
                (self.box_true_start[0], self.box_true_start[1], self.box_true_end[0], self.box_true_end[1]))
            ...
            self.targets[self.target_cursor] = (
            self.box_true_start[0], self.box_true_start[1], self.box_true_end[0], self.box_true_end[1])
            dbh.execute("insert into targets (file, x1, y1, x2, y2) values (?, ?, ?, ?, ?)",
                        (files[self.image_pointer],
                         self.box_true_start[0],
    ...
    def DeleteCurrent(self):
        ...
        if self.target_cursor > -1:
            ...
            if really:
                dbh.execute("delete from targets where file=? and x1=? and y1=?",
                            (files[self.image_pointer], self.targets[self.target_cursor][0],
                             self.targets[self.target_cursor][1]))
                conn.commit()
                self.ReloadTargets()
